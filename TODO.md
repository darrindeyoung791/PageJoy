...
*   [x] 创建 `backend/` 目录。
*   [x] 选择并初始化 Python 后端框架 (FastAPI 或 Django)。
*   [x] 配置 Python 依赖管理 (`requirements.txt` 或 `pyproject.toml`)。
*   [ ] 配置代码质量工具 (black, isort, flake8/ruff)。
*   [ ] 配置测试框架 (pytest)。
*   [ ] 配置环境变量管理。
*   [x] 初始化 Git 仓库 (如果尚未初始化)。
*   [x] 更新 `.gitignore` 文件，排除虚拟环境、构建产物等。

### 2. 后端开发 (`backend/`)

#### 2.1 数据库设计与模型

*   [x] 根据 `app/README.md` 中的数据库表结构，设计并实现 Python ORM 模型 (SQLAlchemy Models 或 Django Models)。
*   [x] 配置数据库连接。
*   [x] 设置数据库迁移工具 (Alembic 或 Django Migrations) 并创建初始迁移。

#### 2.2 API 设计与实现

*   [x] 设计 RESTful API 端点 (可以先用 OpenAPI/Swagger YAML 在 `shared/` 中定义)。
*   [x] 实现用户认证 API (注册、登录、获取用户信息)。
*   [x] 实现文章相关 API (获取文章列表、获取文章详情、点赞)。
*   [x] 实现创作者相关 API (获取创作者信息、关注/取消关注)。
*   [x] 实现杂志相关 API (获取杂志列表、获取杂志详情)。
*   [x] 实现会员订阅相关 API (获取订阅信息、处理支付Webhook - 模拟或集成)。
*   [x] 实现收藏相关 API (获取收藏列表、添加/移除收藏)。
...
*   [x] 配置 Flutter 项目，添加必要的依赖 (http client, state management, local db)。
*   [x] 设计并实现 App 的整体 UI/UX (Material Design 3)。
*   [ ] 实现 Splash Screen (如果需要)。
*   [x] 实现 Navigation Rail/Bar 切换逻辑。
*   [x] 实现主页瀑布流布局。
*   [x] 实现文章阅读页面。
*   [x] 实现创作者页面。
*   [x] 实现杂志页面。
...
*   [ ] 实现底部 FAB 的显示/隐藏逻辑。
*   [x] 实现内容渐变遮罩效果。
*   [x] 集成 `http` 客户端，调用后端 API。
*   [ ] 实现本地数据存储 (SQLite) 用于缓存或离线数据。
*   [x] 实现状态管理，管理用户信息、文章数据等。
