...
from models.models import UserSubscription as UserSubscriptionModel
...
def create_user_subscription(subscription: UserSubscriptionCreate, db: Session = Depends(get_db)):
    db_subscription = UserSubscriptionModel(**subscription.dict())
...
def read_user_subscription(subscription_id: int, db: Session = Depends(get_db)):
    db_subscription = db.query(UserSubscriptionModel).filter(UserSubscriptionModel.id == subscription_id).first()
...
def read_user_subscriptions(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    subscriptions = db.query(UserSubscriptionModel).offset(skip).limit(limit).all()
