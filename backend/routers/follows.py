...
from models.models import UserFollow as UserFollowModel
...
def create_follow(follow: UserFollowCreate, db: Session = Depends(get_db)):
    ...
    existing_follow = db.query(UserFollowModel).filter(
        UserFollowModel.follower_id == follow.follower_id,
        UserFollowModel.followed_id == follow.followed_id
    ).first()
    
    if existing_follow:
        raise HTTPException(status_code=400, detail="Follow relationship already exists")
    
    db_follow = UserFollowModel(**follow.dict())
    db.add(db_follow)
    db.commit()
...
def read_follow(follower_id: int, followed_id: int, db: Session = Depends(get_db)):
    db_follow = db.query(UserFollowModel).filter(
        UserFollowModel.follower_id == follower_id,
        UserFollowModel.followed_id == followed_id
    ).first()
...
def read_followers(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    followers = db.query(UserFollowModel).filter(UserFollowModel.followed_id == user_id).offset(skip).limit(limit).all()
    return followers
...
def read_following(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    following = db.query(UserFollowModel).filter(UserFollowModel.follower_id == user_id).offset(skip).limit(limit).all()
    return following
